...
const Products = () => {
  ...
  const { handleIncrement, handleDecrement, handleLike, handleLoad } = useProductsContextProvider();
  return (
            ...
            <div className={`${styles.handlers}`}>
              <div className={`${styles.like}`}>
                <p>
                  <i
                    ...
                    className={product.liked === true ? "bi bi-heart-fill" : "bi bi-heart"}
                  ></i>
                </p>
              </div>
              ...
              <div className={`${styles.crement}`}>
                <button
                  ...
                  className={`${styles.btn} ${product.quantity <= 1 ? styles.delete : styles.decrement}`}
                >
                  <i className="bi bi-trash"></i>
                  <span>-</span>
                </button>
                <p className={`${styles.quantity}`}>{product.quantity}</p>
                <button onClick={() => handleIncrement(product)} className={`${styles.btn}`}>
                  +
                </button>
              </div>
            </div>
